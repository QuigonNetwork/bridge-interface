...
class TON extends AbstractChain {
  ...
  async preParse(nft) {
    const _contract = nft?.native?.collection?.address || "SingleNFt";
    ...
    return {
      ...
      uri,
      image: nft?.native?.metadata?.image,
      ...
      native: {
        ...nft?.native,
        tokenId: nft?.native?.index,
        contract: _contract,
        name:
          nft?.native?.name ||
          nft?.native?.metadata?.name ||
    ...
    return new Promise((resolve, reject) => {
      const handler = async (
        ...
        if (toChain === this.nonce && fromChain === from) {
          ...
          executedSocket.off("tx_executed_event", handler);
          return resolve(
            targetCanister ? { tagetCanister: targetCanister } : undefined
          );
        }
        resolve(undefined);
        executedSocket.off("tx_executed_event", handler);
